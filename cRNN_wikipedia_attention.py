...
def grad_clipping_by_norm(grads, clip_c):
    g2 = 0.
    new_grads = []
    for g in grads:
        g_norm = tensor.sqrt((g**2).sum())
        new_grads.append(tensor.switch(g_norm > clip_c, g * clip_c/g_norm, g))
    grads = new_grads
    return grads
...
class ParameterInitializator:
    ...
    def global_init_params(self):
        ...
        params = self.cRNN_init_params(options, params, weight_filler, prefix='cRNN_1')
        params = self.cRNN_init_params(options, params, weight_filler, prefix='cRNN_2')
    ...
    def cRNN_init_params(self, options, params, weight_filler, prefix='cRNN'):
        '''
        Init the cRNN parameter:
        '''
        ...
        #W_a = weight_filler.uniform_weight(1, options['attentive_residuals']).astype('float32')
...
class Layers:
    ...
    def dropout_layer(self, state_before, drop_flag, trng):
        retain_prob = 0.5
        drop_prob = 1 - retain_prob
        mask = trng.binomial(state_before.shape, p=drop_prob, n=1, dtype=state_before.dtype)
        proj = tensor.switch(drop_flag, state_before * mask, retain_prob * state_before)
        return proj
    ...
    def cRNN_layer(self, tparams, x_in, h_0, c_0, options, trng,  drop_flag, prefix='cRNN'):
        
        nsteps = x_in.shape[0]
        attentive_residuals = options['attentive_residuals']
        ...
        def _cRNN_unit(x_, h_, c_, h_att):
            ...
            # complementary memory via attentive residuals
            
            
            #h = o * tensor.tanh(c)
            
            w_a = tensor.tensordot(h_, h_att.T)
            #w_a = theano.printing.Print('-----w_a----- \n')(w_a)
            W_norm = tensor.nnet.softmax(w_a)
            a = tensor.tensordot(W_norm, h_att,1)
            #a = theano.printing.Print('-----a----- \n')(a)
            reshaped_a = tensor.reshape(a,(a.shape[1],a.shape[2]))
            
            h = o * tensor.tanh(c + reshaped_a)
            
            # update attentive residual window
        ...
        h_att_list =tensor.alloc(numpy_floatX(0.),attentive_residuals,n_samples, options['dim_hidden'])

        [h, c, h_att_list], updates = theano.scan(_cRNN_unit,
                                    sequences=[x_in],
        ...
        #mask = trng.binomial(h.shape, p=0.5, n=1, dtype=h.dtype)
        #h = tensor.switch(drop_flag, h * mask, 0.5 * h)
        return h, c, updates
...
def build_model(tparams, options):
    ...
    drop_flag = theano.shared(numpy_floatX(1.0))
    ...
    #emb = layer.dropout_layer(emb, drop_flag, trng) 
          
    h_1, c_1, updates = layer.cRNN_layer(tparams, emb, h1_init, c1_init, options, trng,  drop_flag, prefix='cRNN_1') 
    h_2, c_2, updates = layer.cRNN_layer(tparams, h_1, h2_init, c2_init, options, trng,  drop_flag, prefix='cRNN_2')  
     
    fc = layer.mlp_layer(tparams, h_2)
    #fc = layer.dropout_layer(fc, drop_flag, trng) 
    ...
    return drop_flag, x, y, h1_init, c1_init, h2_init, c2_init, h_1[-1], c_1[-1], h_2[-1], c_2[-1], cost, loss, updates
...
def train_cRNN(
    dim_hidden=300,  # hidden units.
    patience=1000,  # Number of epoch to wait before early stop if no progress
    max_epochs=6,  # The maximum number of epoch to run
    max_max_epochs=100,
    dispFreq=100,  # Display to stdout the training progress every N updates
    decay_c=0.00001,  # Weight decay 
    init_lrate=1.0,  
    step_size=[5, 10, 20, 30], # step size to descrease learning rate
    n_words=77001,  # Vocabulary size
    encoder='cRNN',  
    model_path='models/',  # path to save model
    model_name='cRNN_1117_5', #logs
    validFreq=5000,  # Iterations for validation.
    saveFreq=100000,  # Iterations for save
    n_timesteps =20, # Length of sequence 
    batch_size=64,  # Training batch size
    valid_batch_size=64,  # Val/Test batch size
    attentive_residuals = 5,
    ...
    log_path='log/cRNN_1119_wikipedia_standard.log'
):
    ...
    (drop_flag, x, y, h1_init, c1_init, h2_init, c2_init, h1_last, c1_last, h2_last, c2_last, cost,loss, updates) = build_model(tparams, model_options)#, h_, c_
    ...
    f_cost = theano.function([x, y, h1_init, c1_init, h2_init, c2_init], [h1_last, c1_last, h2_last, c2_last, loss], updates=updates, name='f_cost')

    grads = tensor.grad(cost, wrt=list(tparams.values()))
    #pdb.set_trace()
    f_grad = theano.function([x, y, h1_init, c1_init, h2_init, c2_init], grads, updates=updates, name='f_grad')
     
    #f_ppl =  theano.function([x, mask, y], ppl, name='f_ppl')

    if clip_c > 0.:
            print('Applying gradient clipping (clip_c:'+str(clip_c)+')...')
            grads = grad_clipping_by_norm(grads, clip_c)
    ...
    try:
        ...
        for eidx in range(max_max_epochs):
            
            #print('learning rate ',str(lrate) )
            #temp = numpy.maximum(eidx + 1 - max_epochs, 1.0)
            lr_decay = init_lr_decay**numpy.maximum(eidx + 1 - max_epochs, 0.0)
            lrate = (init_lrate * lr_decay).astype('float32')
           
            #print('current learning rate: ', str(lrate))           
            #print('learning rate ',str(lrate) )
            #if eidx in step_size:
            #        lrate=(init_lr_decay*lrate).astype('float32')
