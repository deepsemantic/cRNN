...
class SequenceGenerator():
    ...
    def init_vocabulary(self, min_count=10):
        words_to_count = {}
        ...
        vocab_out_path = '%s/vocabulary_wikipedia.txt' % OUTPUT_DIR
    ...
    def encoding_word(self):
        ...
        val_one_hot_vec=[]
        for word in self.trainset:
            train_one_hot_word = [self.vocabulary[word] if word in self.vocabulary else self.vocabulary[UNK_IDENTIFIER]]
            train_one_hot_vec.append(train_one_hot_word)
        for word in self.testset:    
            test_one_hot_word = [self.vocabulary[word] if word in self.vocabulary else self.vocabulary[UNK_IDENTIFIER]]
            test_one_hot_vec.append(test_one_hot_word)
        for word in self.valset:    
            val_one_hot_word = [self.vocabulary[word] if word in self.vocabulary else self.vocabulary[UNK_IDENTIFIER]]
...
def load_dataset(batch_size, n_timesteps):
    print 'load wikipedia dataset...'

    trainset=read_text('wikipedia/train.txt')
    testset=read_text('wikipedia/test.txt')
    valset=read_text('wikipedia/valid.txt')
